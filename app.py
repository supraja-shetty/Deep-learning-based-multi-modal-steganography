...
# =========================
# =========================
# Logging (temporary for Render)
# =========================
def log_action(action_type, filename=None, details=None):
    # Print log instead of saving to MongoDB
    print(f"[LOG] {datetime.datetime.utcnow()} | Action: {action_type} | File: {filename} | Details: {details}")
...
class VideoRNN(nn.Module):
    ...
    def __init__(self, hidden_size=256, num_classes=2):
        ...
        self.rnn = nn.LSTM(input_size=self.feature_dim, hidden_size=hidden_size, num_layers=1, batch_first=True)
...
def hide_image():
    ...
    try:
        ...
        if secret_pixels > cover_capacity:
            ...
            new_size = (max(1, int(secret_img.width * scale)), max(1, int(secret_img.height * scale)))
...
# =========================
# =========================
# SEND / RECEIVE ROUTES WITH SOCKET.IO
# =========================
...
def send_message():
    ...
    # If text file, prepend password automatically (first line)
    if uploaded.filename.endswith(".txt"):
        content = uploaded.read().decode("utf-8")
        # Check if first line already has password
        if "\n" not in content:
            # No password line, insert dummy
            content = "N/A\n" + content
        # Save as file
        save_path = os.path.join(app.config["UPLOAD_FOLDER"], "send_" + uploaded.filename)
    ...
    # Log correctly with "filename"
    log_action("send", filename=uploaded.filename, details={"path": save_path})

    # Notify via Socket.IO
    socketio.emit("new_file", {"filename": uploaded.filename})

    return jsonify({"message": "File sent successfully", "filename": uploaded.filename})

@app.route("/receive", methods=["GET"])
def receive_message():
    last_entry = logs_col.find_one({"action": "send"}, sort=[("timestamp", -1)])
    if not last_entry or not last_entry.get("filename"):
        return jsonify({"error": "No file received yet"}), 404

    filepath = os.path.join(app.config["UPLOAD_FOLDER"], "send_" + last_entry["filename"])
    if not os.path.exists(filepath):
        return jsonify({"error": "File not found on server"}), 404

    log_action("receive", filename=last_entry["filename"])
